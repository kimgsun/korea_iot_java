...
/*
    === 스트림(Stream, 흐름) API ===
    : 자바 8부터 도입된 기능
        , 컬렉션 또는 배열에 저장된 데이터를 선언형 방식(직관적, 간결)으로 처리할 수 있게 지원하는 도구
...
public class H_Stream {
    public static void main(String[] args) {
        ...
        long count2 = names.stream().distinct().count();
        System.out.println("총 인원 수(중복 제거): " + count2);
        ...
        // 8)
        System.out.println("== 이름 길이 기준 내림차순 정렬 =="); // 글자 수 내림차순
        // names.stream()
        //         .sorted() // 오름차순 정렬(기본) >> 원래 .sorted((a, b) -> a.length() - b.length())
        //         .forEach(System.out::println);
